...
impl TextArea {
    ...
    pub fn get_prefix_width(&self) -> usize {
        self.get_prefix().width()
    }

    fn get_placeholder_text(&self) -> &'static str {
        use crate::services::placeholder_prompts::get_placeholder_prompt;
        get_placeholder_prompt(self)
    }

    /// Custom render method that can handle password masking
    pub fn render_with_state(
        ...
        self.render_lines(area, buf, &lines, start..end, waiting_for_shell_input);
    }
    ...
    pub fn input(&mut self, event: KeyEvent) {
        match event {
            ...
            _o => {
                // Unhandled key event - could add debug logging here if needed
            }
        }
    }
        ...
        // Ensure cache is ready (potentially mutably borrow, then drop)
        {
            ...
            if needs_recalc {
                ...
                let lines = if width > 0 {
                    ...
                    // If no space for content, create empty lines
                    Vec::new()
                };
...
impl StatefulWidgetRef for &TextArea {
    ...
    fn render_ref(&self, area: Rect, buf: &mut Buffer, state: &mut Self::State) {
        ...
        let start = scroll as usize;
        let end = (scroll + area.height).min(lines.len() as u16) as usize;
        self.render_lines(area, buf, &lines, start..end, false);
    }
}
...
impl TextArea {
    fn render_lines(
        ...
        if self.text.is_empty() && self.session_empty {
            ...
            // Draw placeholder text after the cursor
            let placeholder_x = content_x + 1; // Start after the cursor
            if placeholder_x < area.x + area.width {
                buf.set_string(
                    placeholder_x,
                    y,
                    placeholder,
        ...
        for (row, idx) in range.clone().enumerate() {
            ...
            // Draw base line with default style, offset by prefix width
            let content_x = area.x + prefix_width;

            // Mask input if waiting for shell input (like password prompts)
            let display_text = if waiting_for_shell_input && self.shell_mode {
                "*".repeat(self.text[line_range.clone()].len())
            } else {
                self.text[line_range.clone()].to_string()
            };

            buf.set_string(content_x, y, &display_text, Style::default());

            // Overlay styled segments for elements that intersect this line (only if not masked)
            if !waiting_for_shell_input || !self.shell_mode {
        ...
        // Render cursor
        self.render_cursor(area, buf, lines, range);
    }
    ...
    fn render_cursor(
        ...
        if let Some(cursor_line_idx) = Self::wrapped_line_index_by_start(lines, self.cursor_pos) {
            // Check if cursor line is in the visible range
            if cursor_line_idx >= range.start && cursor_line_idx < range.end {
                let display_row = cursor_line_idx - range.start;
                let y = area.y + display_row as u16;

                // Calculate cursor column position, accounting for prefix
                let line_range = &lines[cursor_line_idx];
                let cursor_col = self.text[line_range.start..self.cursor_pos].width() as u16;
                let prefix_width = self.get_prefix_width() as u16;
                let x = area.x + prefix_width + cursor_col;

                // Draw cursor as a block character
